...
void insert_header_file_clogan(cLogan_model *loganModel) {
    ...
    char *log = "blank mark,clogan header";
    int flag = 1;
//    long long local_time = get_system_current_clogan();
    char timeStr[32] = {0};
    get_local_cur_data(timeStr);

    char *thread_name = "clogan";
    long long thread_id = 1;
    int ismain = 1;
    Construct_Data_cLogan *data = construct_json_data_clogan(log, flag, timeStr, thread_name,
                                                             thread_id, ismain);
    if (NULL == data) {
        return;
    }
...
void get_local_cur_data(char *time_str) {
    time_t iNowTime = 0;
   ...
   pstNowTransfer->tm_min, pstNowTransfer->tm_sec, 0);
}
